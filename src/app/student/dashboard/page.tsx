                ...
                <div className="bg-white rounded-xl shadow p-6">
                  ...
                  <div className="space-y-2">
                    {currentWeek.weekTopics.map((weekTopic, index) => {
                      // Find progress data for this topic
                      const topicProgress = progress.find(p => 
                        p.topicId === weekTopic.assignment.topic.id || 
                        p.assignmentId === weekTopic.id
                      )
                              ...
                              {topicProgress && (
                                <div className="text-right">
                                  <div className="text-xs text-gray-600">
                                    {topicProgress.totalCount > 0 ? Math.round((topicProgress.solvedCount / topicProgress.totalCount) * 100) : 0}%
                                  </div>
                                  <div className="w-16 bg-white rounded-full h-1.5">
                                    <div
                                      className="bg-green-500 h-1.5 rounded-full transition-all"
                                      style={{ 
                                        width: `${topicProgress.totalCount > 0 ? Math.round((topicProgress.solvedCount / topicProgress.totalCount) * 100) : 0}%` 
                                      }}
                                    ></div>
                                  </div>
                                </div>
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    {topicProgress && (
                                      <div className="text-xs opacity-75 mt-1">
                                        📊 {topicProgress.solvedCount}/{topicProgress.totalCount} soru
                                      </div>
                                    )}
                                  </div>
                                </div>
